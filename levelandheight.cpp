...
int dfs(int node,int parent,int level,vector<vector<int>> &adj){
    cout<<"level of "<<node<<": "<<level<<endl;
    int height = 0;
    for(auto &v : adj[node]){
        if(v != parent){
            height = max(dfs(v,node,level+1,adj)+1,height);
        }
    }
    cout<<"heigth of "<<node<<": "<<height<<endl;
    return height;
}
